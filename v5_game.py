...
spin_images = [
    pygame.image.load("resources/images/spin1.png").convert_alpha(),
    pygame.image.load("resources/images/spin2.png").convert_alpha(),
    pygame.image.load("resources/images/spin3.png").convert_alpha(),
    pygame.image.load("resources/images/spin4.png").convert_alpha(),
    pygame.image.load("resources/images/spin5.png").convert_alpha(),
    pygame.image.load("resources/images/spin6.png").convert_alpha(),
    pygame.image.load("resources/images/spin5.png").convert_alpha(),
    pygame.image.load("resources/images/spin4.png").convert_alpha(),
    pygame.image.load("resources/images/spin3.png").convert_alpha(),
    pygame.image.load("resources/images/spin2.png").convert_alpha(),
...
#---------------------------- gameplay page -----------
img_size = (500, 500)
pixel_btn_surface = pygame.transform.scale(pygame.image.load("btn4.png"),img_size).convert_alpha()
pixel_btn_rect = pixel_btn_surface.get_rect(midbottom = (100, 200))

spin_img_size = (500, 500)
spin_btn_surface = pygame.transform.scale(pygame.image.load("resources/images/spin1.png"),img_size).convert_alpha()
spin_btn_rect = spin_btn_surface.get_rect(midbottom = (500, 500))
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pixel_btn_rect.collidepoint(event.pos):
                pixel_btn_surface = pygame.transform.scale(pygame.image.load("btn3.png"),img_size)

            elif spin_btn_rect.collidepoint(event.pos) and not is_spinning:
                is_spinning = True
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if game_state == "menu":
                ...
                elif pixel_btn_rect.collidepoint(event.pos):
                    pixel_btn_surface = pygame.transform.scale(pygame.image.load("btn4.png"),img_size)
                elif spin_btn_rect.collidepoint(event.pos):
                    current_spin_index = (current_spin_index + 1) % len(spin_images)
...
#drawing (displaying sprites/objects)
    ...
    #gameplay page    fullscreen_w, fullscreen_h
        ...
        screen.fill(RED)
        pygame.draw.rect(screen, GREY,(50, 0, 350, 1070))
        ...
        screen.blit(pixel_btn_surface, pixel_btn_rect)
        screen.blit(spin_images[current_spin_index], spin_btn_rect)
        screen.blit(quit_btn_surface, quit_btn_rect)
    ...
    # Handle spin animation frame updates
    if is_spinning:
        now = pygame.time.get_ticks()
        if now - spin_start_time >= spin_frame_duration:
            spin_start_time = now
            current_spin_index += 1
            if current_spin_index >= len(spin_images):
                current_spin_index = len(spin_images) - 1  # stay on final image
                is_spinning = False
    clock.tick(FPS)
